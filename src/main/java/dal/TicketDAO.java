...
public class TicketDAO {
    ...
    public List<Integer> getListSeatsByTicketFilmId(int ticket_filmid, int status) {
        ...
        String sql = "SELECT seatid FROM ticket t WHERE t.ticket_filmid = ? and t.status=?";
        try {
    ...
    public int insertTicket(int accountId, String seatId, String ticketFilmId, int status) {
        ...
        for (String si : seatIds) {
            String sql = "INSERT INTO ticket (accountid, seatid, ticket_filmid,bookingTime,status) VALUES (?, ?, ?,?,?)";
    ...
    public int removeTicket(int accountId, String seatId, String ticketFilmId) {
        String sql = "DELETE FROM ticket WHERE accountid = ? AND seatid = ? AND ticket_filmid = ?";
    ...
    public boolean isRecordExist(int accountId, String seatId, String ticketFilmId) {
        String sql = "SELECT 1 FROM ticket WHERE accountid = ? AND seatid = ? AND ticket_filmid = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, accountId);
            ps.setString(2, seatId);
   ...
   public List<TicketHistory> getListHistoriesByAccountId(int accountId) {
        ...
        String sql = "SELECT tf.id , s.seat_number, t.price_ticket, f.title, r.name, e.startHour,e.date,t.bookingTime\n"
                + "FROM ticket t,seat s,ticket_film tf, film f, room r,event e \n"
    ...
    public int deleteTicketsWithStatusZero(int accountId) {
        ...
        String sql = "DELETE FROM ticket WHERE status = 0 AND accountid = ?";
        int rowsAffected = 0;
    ...
    public int insertTicket1(int accountId, String seatId, String ticketFilmId, int status, int price_ticket) {
        ...
        for (String si : seatIds) {
            String sql = "INSERT INTO ticket (accountid, seatid, ticket_filmid, bookingTime, status, price_ticket) VALUES (?, ?, ?,?,?, ?)";
